...
- 右上角工具条 = 宿主内置项 + 插件条目（`ctx.header`），顺序与显隐由设置页「顶栏工具」编排；插件轮播条目不展示图标，收起态为 Swiper 式上下滑动切换，视窗 171px 全固定 = 名称上限 5 字（60）+ 数值列 111（按 `99999.99 +9999.99%` 最坏宽度冗余预留），两列位置恒定、不推挤相邻按钮
