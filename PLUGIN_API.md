...
保存成 `.js`（推荐连同 `manifest.json` / `README.md` 打成 `.zip` 产物包）→ 设置 → 插件（或插件工坊）→ 安装插件 → 选 zip 包（可多选 / 拖拽，选中即自动解析）/ 粘贴 / 选文件 → 确认安装。包格式见 `PLUGIN_WIKI.md` §8.2。

> 多个 zip 包可以一次传进去：每个包各占一张卡片并行解析，你能看到它停在六步里的哪一步（解包 → 静态预检 →
> 加载产物 → 结构校验 → 清单一致性 → 包间冲突），失败的那张卡会整个包红框并写明原因。
> 若同一批里两个包解析出相同插件 id，按上传顺序先到先得，后到的那个会被告知「和谁撞了、该怎么办」——
> 想换一个版本就先移除前面的那个，别指望静默覆盖。
