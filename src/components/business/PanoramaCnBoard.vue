...
const fetchBoards = async (): Promise<void> => {
  ...
  try {
    ...
    const list =
      activeTab.value === 'industry' ? await fetchIndustryBoards() : await fetchConceptBoards();
    boards.value = list;
...
const sortedBoardsFull = computed(() => {
  ...
  return filtered.sort(
    (a, b) =>
      (sortKey.value === 'totalMarketCap' ? (b.totalMarketCap ?? -Infinity) : (b.changePercent ?? -Infinity)) -
      (sortKey.value === 'totalMarketCap' ? (a.totalMarketCap ?? -Infinity) : (a.changePercent ?? -Infinity)),
  );
});
...
} = useLazyRows<IndustryBoardConstituent>(() => constituents.value, CONSTITUENTS_CHUNK_SIZE);
...
<template>
  <div class="space-y-4">
    ...
    >
      ...
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div class="grid grid-cols-2 gap-2 @3xl:grid-cols-4">
          <div class="rounded-lg bg-up-weak px-3 py-1.5">
            ...
            <p class="text-sm font-semibold tabular-nums text-up">{{ boardStats.upStrong }}</p>
          </div>
          <div class="rounded-lg bg-flat-weak px-3 py-1.5">
            <p class="text-[10px] text-text-tertiary">涨幅≥0%</p>
            <p class="text-sm font-semibold tabular-nums text-up">{{ boardStats.upAll }}</p>
          </div>
          <div class="rounded-lg bg-flat-weak px-3 py-1.5">
            <p class="text-[10px] text-text-tertiary">跌幅&lt;3%</p>
            <p class="text-sm font-semibold tabular-nums text-down">{{ boardStats.downMild }}</p>
          </div>
          <div class="rounded-lg bg-down-weak px-3 py-1.5">
            <p class="text-[10px] text-text-tertiary">跌幅&gt;3%</p>
            <p class="text-sm font-semibold tabular-nums text-down">{{ boardStats.downStrong }}</p>
          </div>
        </div>
      ...
      <div v-else-if="sortedBoardsFull.length > 0">
        <BaseTable
          ...
          min-width="760px"
          row-clickable
          :footer-text="boardsHasMore ? `已展示 ${sortedBoards.length} / 共 ${boardsTotal}，继续滚动加载更多` : undefined"
          @row-click="onSelectBoard"
          @scroll="onBoardsScroll"
        >
          <template #price="{ row }">
            <span class="text-text-secondary">{{ formatPrice(row.price) }}</span>
          </template>
          ...
          <template #totalMarketCap="{ row }">
            <span class="text-text-secondary">{{ formatYuan(row.totalMarketCap) }}</span>
          </template>
          ...
          <template #leader="{ row }">
            <span class="text-text-secondary">{{ row.leadingStock ?? '--' }}</span>
            <span
              class="ml-1 text-xs"
              :class="TREND_TEXT_CLASS[getTrendByChangePercent(row.leadingStockChangePercent ?? 0)]"
            >
    ...
    <!-- 成分股 -->
    <BaseCard v-if="selectedBoard" :title="`${selectedBoard.name} · 成分股（${constituents.length}）`">
      ...
      <div v-else-if="constituents.length > 0">
        <BaseTable
          ...
          min-width="560px"
          row-clickable
          :footer-text="constituentsHasMore ? `已展示 ${visibleConstituents.length} / 共 ${constituentsTotal}，继续滚动加载更多` : undefined"
