...
export const BOARD_PROFIT_BUBBLE = {
  ...
  MIN_HEIGHT: 380,
  /**
   * 最大高度（viewBox 单位）：板块过多堆叠超高时按比例收缩气泡半径适配此高度；
   * 收缩到下限仍超高则由 SVG 的 max-h 等比缩放兜底（不出现滚动条）。
   * 按弹窗 max-w-6xl 内容宽 ≈ 1112px（缩放系数 ≈ 0.87）折算，视觉高度 ≈ 556px
   */
  MAX_HEIGHT: 640,
  /** 半径下限 */
  MIN_RADIUS: 14,
