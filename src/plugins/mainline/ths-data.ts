 ...
 * @returns 清单页 HTML
 */
const fetchBoardListHtml = async (): Promise<string> => {
 ...
 */
const fetchBoardListPageHtml = async (page: number): Promise<string> => {
...
const tryFetchBoardListPage = async (page: number): Promise<BoardSnapshot[] | null> => {
  try {
    ...
    return parseBoardListSnapshotHtml(await fetchBoardListPageHtml(page));
  } catch (error) {
