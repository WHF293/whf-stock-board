...
import type { BoardDaily, BoardSnapshot, EmMappingResult, ThsBoardRef } from './types';
...
const fetchEmBoardListPage = async (
  ...
  for (let attempt = 0; attempt < EM_FETCH_ATTEMPTS; attempt += 1) {
    if (attempt > 0) await delay(EM_SCAN_DELAY_MS);
    try {
      const response = await proxyFetch(url, { headers: { Referer: EM_REFERER } });
      if (!response.ok) {
 ...
 */
export const fetchEmBoardUniverse = async (): Promise<BoardSnapshot[]> => {
  let lastShort = '';
  for (let round = 0; round < EM_UNIVERSE_ROUNDS; round += 1) {
    if (round > 0) await delay(EM_SCAN_RETRY_DELAY_MS);
    const merged = new Map<string, BoardSnapshot>();
    let total: number | null = null;
    for (let page = 1; page <= EM_BOARD_LIST_MAX_PAGES; page += 1) {
      // 频率红线：东财突发限速，同上游连续请求之间留间隔
      if (page > 1) await delay(EM_SCAN_DELAY_MS);
      const pageData = await fetchEmBoardListPage(page);
 ...
 */
export const fetchEmBoardKline = async (emCode: string, year: number): Promise<BoardDaily[]> => {
