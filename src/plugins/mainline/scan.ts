 ...
 * @returns 清单解析结果
 */
const resolveBoardRefs = async (snapshot: MainlineSnapshot): Promise<RefResolution> => {
  try {
 ...
 */
const fetchAllBoardSeries = async (options: BoardFetchOptions): Promise<BoardFetchResult> => {
...
const runEmScan = async (
  ...
  const emBoards = await fetchEmBoardUniverse();
  ...
  const result = await fetchAllBoardSeries({
    refs: mappedRefs,
    existingByCode,
    source: MAINLINE_SOURCE.EM,
    fetchDays: (ref) => {
      const emCode = mapped.get(ref.code)?.code ?? '';
      return fetchEmBoardKline(emCode, year);
    },
...
export const runMainlineScan = async (
  ...
  const { refs, rows, refsSource, listError } = await resolveBoardRefs(snapshot);
  ...
  const ths = await fetchAllBoardSeries({
    refs,
    existingByCode: thsExisting,
    source: MAINLINE_SOURCE.THS,
    fetchDays: (ref) => fetchThsBoardKline(ref.code, year),
  ...
  if (thsAllFailed) {
    ...
    try {
      const em = await runEmScan(refs, year, emExisting, onProgress);
      boards = em.boards;
