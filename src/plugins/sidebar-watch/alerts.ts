 ...
 */
export const describeAlertRule = (rule: WatchAlertRule): string => {
  ...
  if (rule.kind === WATCH_ALERT_KIND.PRICE) {
    return `${WATCH_ALERT_DIRECTION_PRICE_LABEL[direction]} ${formatPrice(rule.value)}`;
  }
  return `${WATCH_ALERT_DIRECTION_CHANGE_LABEL[direction]} ${formatPercentUnsigned(rule.value)}`;
};
...
export const buildAlertNotice = (
  ...
  const target =
    rule.kind === WATCH_ALERT_KIND.PRICE
      ...
      : formatPercentUnsigned(rule.value);

  return {
    title: `${name} ${label} ${target}`,
    body: `现价 ${formatPrice(snapshot.price)}（${formatPercent(snapshot.changePercent)}）`,
    // 提醒语气由「触发方向」决定：向上触发用涨色，向下触发用跌色
    tone: rule.above ? NOTIFY_TONE.UP : NOTIFY_TONE.DOWN,
  };
};
