...
/** 已配置的阈值简述（未配置时为空串） */
const currentSummary = computed(() => describeAlertRule(props.rule));
...
<template>
  <div class="space-y-4">
    ...
    <div>
      ...
      <BaseInput v-model="valueText" :placeholder="valuePlaceholder" />
    </div>
    ...
    <div class="flex items-center justify-between gap-2 pt-1">
      ...
      <span v-else />
      <BaseButton variant="primary" :disabled="!canSave" @click="onSave">保存</BaseButton>
    </div>
  </div>
</template>
