 ...
 *
 * 定位：市面上「股息排行」给出的股息率是**已实施分红的静态口径**（东财 clist `f133`
 * 为近 12 个月已派息 ÷ 现价的 TTM 口径）——它反映的是「过去」。本插件在同一张
 * 排行表上叠加**今年推算**：用今年中报 ÷ 去年中报的净利比作为全年增长系数，
 * 保留去年分红率，推算「今年若维持分红率、全年净利按中报增速走」的股息率。
 *
 ...
 *   同一报告期可能有多条（中期 + 年度各自成行），每股派息须**按代码求和**。
 */
...
/**
 * 排行排序字段：`f133` = 股息率（TTM，东财口径）
 *
 * ⚠️ f133 随现价实时变化（股息率 = 派息 ÷ 现价），逐页串行翻页时行会在页边界漂移
 * （与 dsh-mainline 踩过的 `fid=f3` 丢行是同一机理）。但这里**排行只用来选样本池**，
 * 界面展示的股息率全部由本插件自算（去年股息率 / 推算股息率），
 * 漂移最多导致池边界的个别股票进出、不会静默丢行（按代码去重 + 收不满多翻一页）。
 */
...
export const EM_DIVIDEND_COLUMNS =
  'SECURITY_CODE,SECURITY_NAME_ABBR,REPORT_DATE,ASSIGN_PROGRESS,PRETAX_BONUS_RMB,TOTAL_SHARES';
...
export const DIVIDEND_PAGE_SUBTITLE =
  'TTM 股息率排行（东财口径）+ 今年推算：以「今年中报 ÷ 去年中报」的净利比作全年增长系数、保留去年分红率，推算若维持分红习惯的今年股息率。辅助研究工具，推算不构成投资建议。';
 ...
 * 免责声明（页脚固定展示）
 *
 * 三条口径必须写明：TTM 股息率是东财口径（近 12 个月已实施派息 ÷ 现价）；
 * 去年股息率与推算股息率是本插件自算口径；推算是线性外推不是预告。
 *
 * 末条是跨平台口径警告：集思录等第三方「静态股息率」按**分红到账的自然年**归集，
 * 本页「去年股息率」按**财务报告期**（年度分红方案）归集，实测同一只股可差近一倍
 * （达仁堂 3.71% vs 6.78%、汇洁股份 6.22% vs 11.05%）→ 对比前必须对齐口径。
 */
export const DIVIDEND_DISCLAIMER =
  '本页为历史数据推算工具，不构成投资建议：分红以公司股东大会决议为准，分红率与增速的外推都可能失真；' +
  '「TTM股息率」为东财口径（近 12 个月已实施派息 ÷ 现价），「去年股息率 / 推算股息率」为本页自算口径，两者不可混用。' +
  '注意跨平台口径差异：集思录等平台的「静态股息率」按分红的到账自然年归集，本页按财务报告期（年度分红方案）归集，' +
