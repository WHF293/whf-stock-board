...
const loadAll = async (): Promise<void> => {
  if (activeTab.value === 'dragon-tiger' && dragonItems.value.length === 0) {
    await loadDragon();
  } else if (activeTab.value === 'block-trade' && blockItems.value.length === 0) {
    await loadBlock();
  }
};
...
const dragonDateOptions = computed(() => {
  const dates = [...new Set(dragonItems.value.map((item) => item.date))].sort().reverse();
  return dates.map((date) => ({ label: date, value: date }));
});

/** 大宗交易日期选项 */
const blockDateOptions = computed(() => {
  const dates = [...new Set(blockItems.value.map((item) => item.date))].sort().reverse();
  return dates.map((date) => ({ label: date, value: date }));
});
...
watch(dragonDateOptions, (options) => {
  if (options.length > 0 && !options.some((option) => option.value === dragonDate.value)) {
    dragonDate.value = options[0].value;
  }
});
watch(blockDateOptions, (options) => {
  if (options.length > 0 && !options.some((option) => option.value === blockDate.value)) {
...
const dragonRowsFull = computed(() =>
  dragonItems.value.filter((item) => !dragonDate.value || item.date === dragonDate.value),
);
const {
  rows: dragonRows,
  total: dragonTotal,
...
} = useLazyRows<DragonTigerDetailItem>(() => dragonRowsFull.value, LIST_CHUNK_SIZE);

/** 选中日期的大宗明细（懒加载） */
const blockRowsFull = computed(() =>
  blockItems.value.filter((item) => !blockDate.value || item.date === blockDate.value),
);
const {
  rows: blockRows,
  total: blockTotal,
...
} = useLazyRows<BlockTradeDetailItem>(() => blockRowsFull.value, LIST_CHUNK_SIZE);
      ...
      <template v-if="activeTab === 'dragon-tiger'">
        ...
        <template v-else-if="dragonItems.length > 0">
          <div class="mb-3 flex items-center gap-2">
            <span class="text-xs text-text-tertiary">上榜日期</span>
            <select
              v-model="dragonDate"
              class="rounded-lg bg-flat-weak px-2 py-1 text-xs text-text outline-none"
            >
              <option v-for="option in dragonDateOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>
            <span class="text-xs text-text-tertiary">共 {{ dragonRows.length }} 只上榜（总 {{ dragonTotal }}）</span>
          </div>
          <div @scroll="onDragonScroll">
            <BaseTable
              ...
              min-width="820px"
              row-clickable
              :footer-text="dragonHasMore ? `已展示 ${dragonRows.length} / 共 ${dragonTotal}，继续滚动加载更多` : undefined"
            ...
            >
              ...
              <template #close="{ row }">
                <span class="text-text-secondary">{{ formatPrice(row.close) }}</span>
              </template>
              ...
              <template #netBuyAmount="{ row }">
                <span
                  class="font-medium"
                  :class="(row.netBuyAmount ?? 0) >= 0 ? 'text-up' : 'text-down'"
                >
                  {{ formatYuanWithSign(row.netBuyAmount) }}
                </span>
              </template>
              <template #netBuyRatio="{ row }">
                <span class="text-text-secondary">{{ formatPercent(row.netBuyRatio) }}</span>
              </template>
              <template #reason="{ row }">
                <span class="block max-w-56 truncate text-text-secondary" :title="row.reason">
                  {{ row.reason }}
                </span>
              </template>
              <template #afterChange5d="{ row }">
                <span :class="TREND_TEXT_CLASS[getTrendByChangePercent(row.afterChange5d ?? 0)]">
                  {{ row.afterChange5d === null ? '--' : formatPercent(row.afterChange5d) }}
                </span>
              </template>
            </BaseTable>
          </div>
        ...
        <template v-else-if="blockItems.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <select
              ...
              class="rounded-lg bg-flat-weak px-2 py-1 text-xs text-text outline-none"
            >
              <option v-for="option in blockDateOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>
          ...
          <div @scroll="onBlockScroll">
            <BaseTable
              ...
              min-width="820px"
              row-clickable
              :footer-text="blockHasMore ? `已展示 ${blockRows.length} / 共 ${blockTotal}，继续滚动加载更多` : undefined"
              @row-click="(item) => openDetail(item.code)"
            >
              <template #name="{ row }">
                <span class="font-medium text-text">{{ row.name }}</span>
              ...
              <template #dealPrice="{ row }">
                <span class="text-text-secondary">{{ formatPrice(row.dealPrice) }}</span>
              </template>
              <template #dealVolume="{ row }">
                <span class="text-text-secondary">
                  {{ row.dealVolume === null ? '--' : (row.dealVolume / 10_000).toFixed(2) }}
                </span>
              </template>
              ...
              <template #premiumRate="{ row }">
                <span class="text-text-secondary">{{ formatPercent(row.premiumRate) }}</span>
              </template>
              <template #buyBranch="{ row }">
                <span class="block max-w-52 truncate text-text-secondary" :title="row.buyBranch">
                  {{ row.buyBranch || '--' }}
                </span>
              </template>
              <template #sellBranch="{ row }">
                <span class="block max-w-52 truncate text-text-secondary" :title="row.sellBranch">
                  {{ row.sellBranch || '--' }}
                </span>
              </template>
            </BaseTable>
