    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ...
          [
            '任务栏盯盘小组件',
            '设置三选一（默认关：关闭 / 常驻 / 智能开启），选择常驻或智能开启后，Windows 任务栏上方常驻一个置顶盯盘迷你条（可拖动，位置会记住）：轮播自选盯盘标的的名称 / 现价 / 涨跌幅，阈值触发带提示点。单击迷你条展开气泡看全部候选；点气泡里的标的（或双击迷你条当前标的）会自动唤起主窗口并打开该股详情页，左侧列表就是盯盘候选。智能开启仅在交易日盘中（9:30-15:00，含午休）显示，盘前、盘后与节假日自动隐藏。仅桌面端；迷你条不产生任何额外行情请求（复用盯盘引擎数据），主窗口隐藏到托盘后照常刷新。',
          ],
          [
            '速记',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          {
            term: '任务栏盯盘小组件（桌面端）',
            desc: '三态选择（默认关闭）：关闭 / 常驻 / 智能开启（仅交易日盘中 9:30-15:00 显示，盘前、盘后与节假日自动隐藏）。选择常驻或智能开启后，Windows 任务栏上方常驻盯盘迷你条，轮播自选盯盘标的；单击展开气泡看全部候选，点标的（或双击迷你条当前标的）自动唤起主窗口并打开详情页。显示模式二选一：常驻显示 / 鼠标离开 3 秒自动隐藏（移到屏幕右下角即可唤回）。浏览器开发模式不生效。',
          },
          {
            term: '全局水印（开关）',
