...
const GUIDE_CHAPTERS: GuideChapter[] = [
    ...
    blocks: [
      {
        ...
        items: [
          {
            term: '左侧导航',
            desc: '全站页面入口（市场总览、市场榜单、行情全景、板块日历、自选股、选股器、热点新闻、账户管理、插件工坊）。底部是「设置」入口，点开为右侧抽屉。插件也会往这里加页面（「股票主线」就是插件贡献的）。要注意「装插件」分两个层面：随应用分发的**内置插件**（自选盯盘、速记、股息筛选）只能启停；以 zip 包**安装进来**的插件（如股票主线）则可在插件工坊里卸载 —— 详见「插件体系与开放接口」章节。',
          },
          {
            term: '顶栏',
      ...
      },
      {
        ...
        items: [
          ...
          '侧栏顺序可自定义：设置 →「布局编排 → 路由顺序编排」拖拽排序，弹窗内「恢复默认」可一键复原，确认后立即生效并记住。',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          {
            term: '列表 / 气泡图切换',
            desc: '工具栏按钮组在「列表」（涨跌停矩阵）与「气泡图」（赚钱效应）两个主区视图间切换。气泡图取最新一个完整快照交易日：气泡面积正比于口径值绝对值，右半区为赚钱效应方向；图内按钮组切换「得分率 / 原始得分」口径，高度固定（板块过多时自动缩小气泡适配，切换口径高度不变，极端聚集时整图等比缩小，不出现滚动条）；气泡图不受「板块过滤器」影响。',
          },
          {
            term: '恢复热门排序 / 刷新',
            desc: '「恢复热门排序」仅在自定义过行序后出现；「刷新」在采集进行中或浏览器开发模式下不可用。',
          },
        ],
      },
      {
  ...
  {
    id: 'bull-review',
    title: '历史复盘：牛熊模式库（行情全景内）',
    tag: '看盘',
    icon: 'trophy',
    intro:
      '入口在「行情全景」页签栏的「历史复盘」子页签（随应用内置提供，显隐与顺序由全景页右上角「配置页签」统一控制）。定位：把 2000 年以来「机构抱团 → 瓦解」的七轮牛市与七轮大级别熊市做标准化拆解，沉淀历史模式用于对标当下——不预测下一轮行情，也不承诺预测崩盘或见底的时点。',
    blocks: [
      {
        type: 'defs',
        items: [
          {
            term: '牛市 / 熊市 / 模式库',
            desc: '面板顶部三个页签切换。牛市页签收录五朵金花（03–04）、股改大牛市（05–07）、四万亿周期牛（08–10）、TMT 杠杆牛（12–15）、核心资产·白酒抱团（16–21）、新能源宁组合（20–22）、本轮科技 AI 抱团（24–至今）七轮；熊市页签收录国有股减持大熊市（01–05）、全球金融危机快熊（07–08）、五年结构长熊（09–13）、杠杆清算熊（15–16）、贸易战去杠杆熊（18）、核心资产长熊（21–24）、本轮科技崩盘熊（26.7–至今，进行中）。',
          },
          {
            term: '行情阶段图',
            desc: '指数月 K 上铺阶段色带并标注顶 / 底：牛市为启动 → 发酵主升 → 高潮鱼尾 → 退潮四段，熊市为初跌 → 反弹中继 → 主跌磨底 → 见底（不同轮次略有裁剪）。色带区间是复盘结论，顶 / 底月份与涨跌幅度由月 K 数据回算。',
          },
          {
            term: '瓦解形态与触发杀法',
            desc: '牛市按瓦解形态分 A（涨停萎缩后批量跌停）/ B（中军先滞涨、一段时间后才大跌）/ C（几乎不跌停、权重阴跌见顶）三类；熊市按触发杀法分 A（流动性急杀）/ B（盈利阴跌磨底）/ C（制度筹码冲击）三类。',
          },
          {
            term: '错配画像',
            desc: '见顶（熊市：见底）前五个维度（赚钱效应 / 中军走势 / 媒体舆论 / 机构行为 / 散户情绪）的背离标记——多维度互相背离即高风险鱼尾区，但只代表「禁加仓」，不代表马上暴跌。',
          },
          {
            term: '事件时间线',
            desc: '每轮约 8–15 条关键节点，按「龙头动向 / 国内政策 / 海外环境 / 资金情绪 / 产业业绩」五类标注，并区分「预期（叙事启动）」与「落地（已兑现）」；重点看中军领涨与见顶时点、多杀多踩踏、政策与流动性拐点的先后次序。',
          },
          {
            term: '模式库',
            desc: '牛 / 熊各一张七轮横向对比表（涨跌幅、回撤、反弹、形态、触发器，可按列排序）、牛市规律卡（抱团自增强循环、瓦解信号五类、鱼尾四条件）与熊市规律卡（三种杀法、政策底 ≠ 市场底、底部四信号、熊市里的结构牛、牛短熊长），以及四轮机构抱团的看板预警能力对比。',
          },
          {
            term: '更新到最新',
            desc: '指数月 K 来自新浪源并落本地库，首次进入自动拉取（每只间隔 0.5 秒），之后重进页面不联网；点「更新到最新」手动补最新月份。',
          },
        ],
      },
      {
        type: 'list',
        items: [
          '读单轮：先看统计条（涨幅 / 回撤 / 时长），再对照阶段色带看行情形态，最后读错配画像理解「为什么瓦解 / 为什么见底」。',
          '读模式库：把当下板块与历史轮次对号入座——机构抱团赛道优先对 B / C 类，题材小票对 A 类；不同类型主线的阈值不能混用。',
          '熊市复盘重点看「三种杀法」的接力次序与政策底 → 市场底的时滞；底部信号四条齐备后出现的「利好」才是转折级利好。',
          '数字与文案分工：涨跌幅、顶 / 底月份一律由数据回算；驱动逻辑、舆论与情绪是定性复盘观点，仅供参考。',
        ],
      },
      {
        type: 'tip',
        text: '本面板能识别「已确认的退潮 / 见底」，不能预测崩盘或见底发生的具体时点；复盘观点仅供参考，不构成投资建议。桌面客户端与浏览器端数据各自独立落库。',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        ...
        items: [
          ...
          '开始对话：左栏「新建对话」，Enter 发送、Shift + Enter 换行；运行中发送键会变成停止按钮。',
          '按需扩展能力：左栏管理 MCP（数据工具接入）与 Skill（专项方法论），并在「访问设置」里授权给哪些 Agent 使用。',
          '多角色协作：在「Agents」里新建 Agent 配置（主模型、子 Agent 编排），再用它发起对话。编辑子 Agent 时可为它勾选「专属技能」与「可用数据源 MCP」（勾选即声明使用并授予访问权限），也可为它指定独立模型（留空则跟随主 Agent 模型）。',
        ],
      },
      {
        type: 'defs',
        items: [
          ...
          {
            term: 'MCP 管理',
            desc: '内置服务器（应用接口 / stock-sdk / 市场数据 / 财务数据）不可删除或编辑，但可开关与授权；远端 MCP 支持「手动填写」（名称、传输方式 Streamable HTTP 或 SSE、URL、请求头）与「粘贴 JSON」（标准 mcpServers 结构）两种添加方式。',
          },
          {
            term: 'Skill 管理',
          ...
          {
            term: '资源授权',
            desc: '每个 MCP / Skill 卡片右侧的滑杆按钮打开「访问设置」：先打开启用开关，再选「全部 Agent 都可以访问」或「精确设置」（逐个勾选主 Agent 与子 Agent）。注意：精确设置里一个都不勾，等于该资源对包括主 Agent 在内的所有 Agent 都不可用，弹窗会有红色警示。编辑子 Agent 时直接勾选「专属技能 / 可用数据源」与这里是同一份授权关系，两边改动互通。',
          },
          {
            term: '金融边界',
            desc: 'Agent 默认只回答金融、股票相关问题；若所用 Agent 配置自带系统提示词，金融边界会叠加在其上，界面有对应说明。',
          },
          {
            term: '工具调用卡片',
  ...
  },
  {
    ...
    icon: 'plug',
    intro:
      '本应用的很多能力不是写死在主程序里的，而是由插件贡献 —— 官方插件（股票主线 / 股息筛选 / 速记 / 自选盯盘）已全部改为 zip 插件包分发，在「插件工坊」里安装、也能一键卸载；行情与数据能力又经 MCP 开放给 AI Agent。这一章说清能力从哪来、怎么管理，并记录插件与开放接口的迭代。',
    blocks: [
      {
        type: 'defs',
        items: [
      ...
      },
      {
        ...
        rows: [
          ...
          [
            '股息筛选',
            '左侧导航「股息筛选」页面：TTM 股息率排行（自算口径）+ 保留去年分红率、按中报净利增速推算今年股息率，并用规则把「伪高股息」标出来；扫描结果落本机插件库，重进页面不重新联网。',
          ],
        ],
      },
      {
        ...
        items: [
          {
            term: '股票主线（插件包分发）',
            desc: '左侧导航「股票主线」页面：用行业板块成交占比分位、量能倍数、价格分位与板块内涨停结构（涨停家数 / 最高连板 / 封板资金）、板块宽度、主力净流入做抱团主线四阶段判定（萌芽 / 确认 / 狂热 / 瓦解），只给阶段标签与风险提示，不含买卖指令。所有指标统一取「最近完整交易日」截面（盘中不写入当日半日数据），板块行情滞后基准日的行会标「滞后 N 日」并排除出阶段判定；扫描结果落本机库，重进页面不重新联网。它不是内置插件，而是以 zip 插件包的形式在安装后挂载（见「开发者指南」的安装步骤）；卸载只需在插件工坊里操作。',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          {
            term: 'Agent 数据接口（MCP）',
            desc: 'Agent 的数据全部来自内置 MCP 服务器：「应用接口」（读写本机功能数据）、「stock-sdk」（行情原始查询）、「市场数据」（现成口径工具：市场概览、资金流、板块涨跌、涨停池、涨跌分布、龙虎榜、大宗交易、全球指数、美股板块、热点新闻等）、「财务数据」（财务摘要、资产负债表、现金流量表、业绩预告）。插件也可以给 Agent 贡献自己的数据工具，停用插件其工具立即对模型不可见；如何接入外部服务见「开发者指南」章节。',
          },
          {
            term: '工具调用可核对',
      ...
      },
      {
        ...
        rows: [
          [
            '待发版（开发线）',
            '官方插件源码迁出主应用：四个官方插件的源码、清单与 zip 产物包统一放进独立仓库 whf-stock-board-plugin（`plugins/<id>/` + `scripts/build-plugins.mjs`），主应用仓库只留插件内核、样式白名单与开发文档。插件侧用一份从主应用同步的宿主契约快照（`host/`，跑 sync-host-contract 刷新）替代对主应用源码的 import，因此主应用改了插件用到的类型或常量，插件仓库会在类型层立刻报错；出包时打包流水线仍会把产物用到的 Tailwind 类回写进主应用的样式白名单，保证插件界面对着宿主产物 CSS 零缺样式。',
          ],
          [
            '待发版（开发线）',
            '官方插件全部改为 zip 插件包分发：股息筛选 / 速记 / 自选盯盘继股票主线之后走「源码级卸载 + 产物包分发」，源码不再编译进应用，改为在「插件工坊」里安装、也能一键卸载（卸载时可选择是否一并删除插件自己的数据表）。为此宿主新增 app:stock-picker（选股弹窗，宿主渲染 + Promise 回传）、app:watchlist（自选股只读）、app:polling（轮询调度）、app:stock-open（打开个股）、panel:close（按 key 关面板）等服务，官方插件的全部宿主依赖改为运行时注入；插件产物用到的 Tailwind 类由打包流水线自动写进样式白名单，保证插件界面对着宿主产物 CSS 零缺样式。',
          ],
          [
            '待发版（开发线）',
            '「插件工坊」转正：从内置插件改为应用自带的正式页面（宿主注册路由与左侧导航），不再出现在插件清单里、也无法被停用，因此永远是「插件页随插件撤销」的可用兜底落点；页面能力不变（内核概览 / 插件清单启停 / 安装卸载 / 贡献点总览 / 生效服务 / 最近事件）。',
          ],
          [
            '待发版（开发线）',
            '插件设置体系：插件清单新增 settings 字段（声明式 fields 或自定义 component），宿主页内 header 恒置「插件设置」齿轮，点击列出已挂载且声明了设置的插件并渲染设置弹窗；运行时值经 ctx.settings 读写（响应式即时生效），持久化在插件 storage 的 settings 键下（即插件 JSON 的 {"settings": {...}} 字段）。首个消费方：股息筛选插件的表格列显隐与顺序编辑。',
          ],
          [
            '待发版（开发线）',
          ...
          [
            '待发版（开发线）',
            'Agent 子代理配置补全：编辑子 Agent 时可为它勾选「专属技能」与「可用数据源 MCP」（一次勾选同时完成声明与授权，保存后运行时真实生效），独立模型配置从「保存了但不生效」修为真实生效（模型已删除时自动回落主 Agent 模型，不支持工具调用的模型在下拉中标注）；Agent 配置编辑弹窗移除无效的「启用 Skills / 启用 MCP」勾选区（授权统一在资源「访问设置」与子 Agent 编辑弹窗完成）。',
          ],
          [
            '待发版（开发线）',
            'Agent 新增「财务数据」内置 MCP（财务摘要 / 资产负债表 / 现金流量表 / 业绩预告四个只读工具，供子 Agent 查基本面）；对话输入框移除「专业模式 / 日常模式」分段控件，专业模式成为唯一模式（金融边界固定生效）。',
          ],
          [
            '待发版（开发线）',
            '同 id 不再拒绝安装：与已装插件同 id = 升级 / 覆盖重装（数据表保留），与内置插件同 id = 接管内置版（启动装配跳过被接管项，卸载后立刻把内置实现挂回来），安装弹窗写明情形、按钮文案随之变化。同时明确「两种安装」的分工：内置插件随应用分发只能启停（页面注明「随应用分发，只能启停，不能卸载」），用户安装的插件标出来源（zip 包 / 粘贴代码）并可卸载；股票主线（dsh-mainline）据此由内置改为 zip 插件包分发。',
          ],
          [
            'v2.5.0',
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          {
            term: '完整 API 清单',
            desc: '上面的表格只是目录：九个贡献点的字段与默认值、ctx.storage / ctx.db / ctx.settings 的用法、宿主提供的六个服务（跳路由 / 打开面板 / 弹浮窗 / 标的搜索 / 应用版本 / 内核自省）与六个内置事件、配额与频率红线，全部写在仓库根目录的 PLUGIN_API.md 里（第三方插件能用的就是其中「契约层」那一层）。',
          },
        ],
      },
      {
        ...
        rows: [
          ...
          ['ctx.settings', '插件设置：清单里声明 settings 字段，宿主顶栏齿轮即渲染设置弹窗，值持久化在插件自己的 settings JSON 里'],
        ],
      },
      {
        type: 'defs',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          {
            term: '主题色',
            desc: '清新绿 / 淡雅蓝 / 活力橙 / 黑金，即时生效并记住选择；黑暗模式下所有主题共用近黑底色（卡片面抬亮一档保持分层），主色自动适配暗色对比度。',
          },
          {
            term: '涨跌颜色',
          ...
          {
            term: '布局编排',
            desc: '两个编排入口合并一卡：「路由顺序编排」拖拽调整左侧导航顺序，确认后立即刷新侧栏；「右上角工具编排」拖拽调整顶栏条目顺序（含插件条目）。两个弹窗内「恢复默认」均可一键复原。注意：顶栏「Agent 分析」条目被关闭后，各页面的「AI 分析」按钮（热点新闻 / 个股详情）一并隐藏。',
          },
          {
            term: '系统日志',
