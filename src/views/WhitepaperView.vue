...
const GUIDE_CHAPTERS: GuideChapter[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'tip',
        text: '行情类榜单在交易时段每 2 分钟刷新一次；资金流类为重接口，进入页签只拉一次。表头排序只作用于已加载的前 100 / 15 条数据，不等价于全市场排序。',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        ...
        items: [
          ...
          {
            term: '图表指标',
            desc: '点图表右上角的指标配置按钮：主图可选 MA（5，10，30）或 BOLL；副图可选 VOL、MACD、BOLL、KDJ、MACD&KDJ，多选则显示多个独立面板。配置会被记住。分时与五日固定为主图 + 均价线 + VOL + MACD。',
          },
          {
            term: '五档盘口',
