    ...
    blocks: [
      {
        ...
        items: [
          ...
          {
            term: '查看历史净流入',
            desc: '板块净流入模块工具条上的入口，打开「板块历史净流入」页：已选行业（曲线视图勾选的行业）的逐日主力净流入双向条形卡，每板块一张，红条向左为净流入、绿条向右为净流出，悬停可看当日数值，卡片头部为历史累计净额。历史数据在本机随使用自动累积——盘中进入板块净流入页签或本页时增量更新，收盘后与非交易日直接读本地不再请求，首次会一次补齐完整历史；每板块最多保留约一年（250 个交易日）。「刷新」按钮可强制更新当日数据。',
          },
          {
            term: '页签配置',
