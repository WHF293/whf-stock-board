...
export interface UseUserPluginsReturn {
   ...
   */
  install: (code: string) => Promise<UserPluginInstallResult>;
  /**
 ...
 * 用户插件（应用内安装）的宿主编排入口
 *
 * 安装：结构校验 → 写持久化（代码原文）→ 内核 `use(origin: 'user')`；
...
export const useUserPlugins = (): UseUserPluginsReturn => {
  ...
  const install = async (code: string): Promise<UserPluginInstallResult> => {
