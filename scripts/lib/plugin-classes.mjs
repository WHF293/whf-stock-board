 ...
 * 它们的 `.vue` 已经不在 `src/` 下 —— 类名能否出现在产物 CSS 里，一度全靠「宿主别处也用了同一个类」。
 * 这份清单把巧合变成保证：`src/assets/styles/theme.css` 用 `@source './plugin-classes.txt'`
 * 把它纳入扫描，于是这些类无论宿主自己用不用，产物 CSS 里都会生成。
 *
 * 约定：清单入库（它本身不是构建产物），增删 / 更新官方插件后重跑打包脚本即自动刷新。
 */
 ...
 */
export const readHostCssClassProbe = (root) => {
 ...
 * @returns 现有类名集合
 */
export const readExistingAllowlist = (root) => {
  const file = path.join(root, CLASS_ALLOWLIST_RELATIVE);
...
/**
 * 把类名清单写进 `src/assets/styles/plugin-classes.txt`
 * @param root 仓库根目录
 * @param classes 完整类名集合（已含历史）
 */
export const writeClassAllowlist = (root, classes) => {
  const file = path.join(root, CLASS_ALLOWLIST_RELATIVE);
  const body = [
    '# 官方插件用到的 Tailwind 类名 —— 由 scripts/build-plugins.mjs 自动生成，勿手改',
    '# 插件源码已不在 src/ 下（改 zip 产物包分发），Tailwind 扫不到它们；',
    '# 靠这份清单经 theme.css 的 @source 进入产物 CSS。详见 AGENTS.md「第三方插件的样式边界」。',
    '',
    [...classes].sort().join('\n'),
    '',
  ].join('\n');
